...
function encodeResistorColors(ohmsString) {
    ...
    let regex = /(\d+[\.]?\d?)([kM]? )ohms/;
    let stringSplitted = ohmsString.match(regex);
    // full, number, [kM\0]
    let value = parseFloat(stringSplitted[1]);

    if (stringSplitted[2] == "k ") {
        value = value * 1000;
    } else if (stringSplitted[2] == "M ") {
    ...
    let string = `${numberToLetter[first]} ${numberToLetter[second]} ${numberToLetter[third]} gold`
